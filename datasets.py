...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston':
        ...
        # Load the image
        img1 = open_file(folder + 'HSI.mat')['muufl_hsi'].astype(np.float32)
        rgb_bands = (59, 40, 23)

        img2 = open_file(folder + 'LiDAR.mat')['muufl_lidar'].astype(np.float32)
        img2 = np.expand_dims(img2, axis=2)    # (349, 1905) --> (349, 1905, 1)

        gt = open_file(folder + 'gt.mat')['muufl_gt']     # Here, the gt file is load for filtering NaN out and visualization.
    ...
    # Filter NaN out
    # ---- unify img2 to (H,W,C) for all datasets (fix MUUFL) ----
    img2 = _ensure_hwc_for_img2(img2, hw_ref=img1.shape[:2], name=f"{dataset_name}.img2")
    # -----------------------------------------------------------
...
def _as_int_label(x):
    ...
    if np.issubdtype(x.dtype, np.floating):
        xf = x[np.isfinite(x)]
        if xf.size and np.max(np.abs(xf - np.rint(xf))) < 1e-6:
            return np.rint(x).astype(np.int64)
    return x

def make_split_masks(gt, ignored_labels=(0,), mode="fraction",
                     train_fraction=0.1, train_per_class=50,
                     seed=0, stratified=True):
    """
    构造 train/test mask（同 shape=(H,W)）
    - train/test mask 只有被选中的点保留原类别 id，其余位置=0
    - ignored_labels（默认0）的位置永远不会被选入
    mode:
      - "fraction": 按比例切分（默认每类分层抽样）
      - "fixed": 每类固定 train_per_class 个训练点，其余全做测试
    """
    rng = np.random.default_rng(seed)
    gt = _as_int_label(gt)

    ignore_set = set(int(x) for x in ignored_labels)
    train = np.zeros_like(gt, dtype=np.int64)
    test  = np.zeros_like(gt, dtype=np.int64)

    # 可选：按类分层（强烈推荐，避免某些类被抽没）
    classes = sorted([int(c) for c in np.unique(gt) if int(c) not in ignore_set])
    ...
    if mode == "fraction":
        if stratified:
            for c in classes:
                ...
                if n == 0:
                    continue
                n_tr = int(np.floor(n * train_fraction))
                n_tr = max(1, n_tr) if train_fraction > 0 else 0
                n_tr = min(n_tr, n)
                tr = idx[:n_tr]
                te = idx[n_tr:]
                train[tr[:,0], tr[:,1]] = c
            ...
            idx = np.argwhere(~np.isin(gt, list(ignore_set)))
            rng.shuffle(idx)
            n = len(idx)
            n_tr = int(np.floor(n * train_fraction))
            tr = idx[:n_tr]
            te = idx[n_tr:]
            train[tr[:,0], tr[:,1]] = gt[tr[:,0], tr[:,1]]
            test[te[:,0], te[:,1]]  = gt[te[:,0], te[:,1]]
        ...
        for c in classes:
            ...
            if n == 0:
                continue
            n_tr = min(int(train_per_class), n)
            tr = idx[:n_tr]
            te = idx[n_tr:]
            train[tr[:,0], tr[:,1]] = c
            test[te[:,0], te[:,1]]  = c
    else:
        raise ValueError(f"Unknown mode: {mode}, use 'fraction' or 'fixed'.")

    return train, test

def save_split_mat(train_mask, test_mask, out_tr_path, out_ts_path,
                   tr_key="TRLabel", ts_key="TSLabel"):
    """保存成 main.py 直接能读的 .mat（带 TRLabel/TSLabel）"""
    savemat(out_tr_path, {tr_key: _as_int_label(train_mask)})
    savemat(out_ts_path, {ts_key: _as_int_label(test_mask)})

def make_and_save_split(dataset_name, folder="./Datasets/",
                        mode="fixed", train_per_class=50,
                        train_fraction=0.1, seed=0,
                        out_prefix="MySplit"):
    """
    一键：加载数据 -> 用 gt 构造 train/test -> 保存 .mat
    说明：
      - Augsburg/Berlin 你现在 get_dataset 里 gt=train_gt+test_gt，正好是“全量标注点集合”
      - Houston/Trento gt 本身就是全量标签
    """
    img1, img2, gt, label_values, ignored_labels, rgb_bands, palette = get_dataset(dataset_name, folder)
    ...
    out_tr = os.path.join(folder, f"{out_prefix}_TRLabel.mat")
    out_ts = os.path.join(folder, f"{out_prefix}_TSLabel.mat")
    save_split_mat(train_mask, test_mask, out_tr, out_ts)

    # 打印个快速确认
    n_tr = int(np.count_nonzero(train_mask))
    n_te = int(np.count_nonzero(test_mask))
    print(f"[OK] saved: {out_tr}")
    print(f"[OK] saved: {out_ts}")
    print(f"train points={n_tr}, test points={n_te}")

    return train_mask, test_mask, out_tr, out_ts

def _ensure_hwc_for_img2(img2, hw_ref=None, name="img2"):
    x = np.asarray(img2)
    x = np.squeeze(x)
    ...
    # (H,W,a,b) -> (H,W,a*b)  (MUUFL: (H,W,1,2)->(H,W,2))
    elif x.ndim == 4:
        if hw_ref is not None and x.shape[0] == hw_ref[0] and x.shape[1] == hw_ref[1]:
            h, w = x.shape[0], x.shape[1]
            x = x.reshape(h, w, -1)
        else:
            # 如果不是 (H,W,*,*)，直接报出来让你看
            raise ValueError(f"{name} unexpected 4D shape {x.shape}, hw_ref={hw_ref}")

    if x.ndim != 3:
        raise ValueError(f"{name} should be (H,W,C), got {x.shape}")
...
class MultiModalX(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        # ===== MUUFL fix: data2 can be (H,W,1,2) =====
        if data2.ndim == 2:
            # (H,W) -> (H,W,1)
            data2 = data2[..., None]
        elif data2.ndim == 4:
            # (H,W,a,b) -> (H,W,a*b)
            h, w, a, b = data2.shape
            data2 = data2.reshape(h, w, a * b)
        # ============================================

        # 现在必须是 (H,W,C)
        if data2.ndim != 3:
            raise ValueError(f"Unexpected data2 patch shape {data2.shape}, full data2 {np.asarray(self.data2).shape}")
