...
def display_lidar_data(img, vis):
    ...
    print("DEBUG gray shape:", np.asarray(gray).shape, "ndim:", np.asarray(gray).ndim, "dtype:", np.asarray(gray).dtype)

    # if gray.ndim == 2:
    #     gray = gray[:, :, None]   # H,W -> H,W,1
    if gray.ndim == 4 and gray.shape[2] == 1:
        gray = gray[:, :, 0, :]     # -> (H, W, 4)
